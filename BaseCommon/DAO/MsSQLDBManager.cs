    ...
    public class MsSqlDBManager<TAction> : IDBManager<TAction>, IDisposable
    {
        ...
        public async Task<DataSet> GetDataSetAsync(string spName, List<CustomParameter> paramData)
        {
            ...
            if (_command != null)
            {
                ...
                {
                    //LogHelper.WriteExceptionToLog(String.Format("errorString: {0}\ne: {1}", e.Message, e.ToString()));
                    throw new BaseDatabaseException("GetDataSetAsync:" + e.Message, e);
                }
            }
        ...
        {
            if (objTable == null) throw new BaseDatabaseException("objTable is null");
            if (tbData == null) throw new BaseDatabaseException("tbData is null");
            if (objTable.KEYCOLUMNS == null) throw new BaseDatabaseException("Action is requiring key column");
            if (objTable.KEYCOLUMNS.Count == 0) throw new BaseDatabaseException("Action is requiring key column");
            //Check DataTable must be contained key column
            foreach (string sCol in objTable.KEYCOLUMNS)
            {
                if (!tbData.Columns.Contains(sCol)) throw new BaseDatabaseException("Data do not contain primary key " + sCol);
            }

            //check if use auto number
            if (objTable.IS_AUTONUMBER_BY_MAX)
            {
                if (string.IsNullOrEmpty(objTable.AUTONUMBER_COLUMN)) throw new BaseDatabaseException("No auto number column were set");
            }
            ...
            {
                //LogHelper.WriteExceptionToLog(String.Format("errorString: {0}\ne: {1}", e.Message, e.ToString()));
                throw new BaseDatabaseException("UpdateTableReturnData:" + e.Message, e);
            }
        ...
        private async Task<DataTable> GetDataTableAsync(string cmd)
        {
            ...
            if (_command != null)
            {
                ...
                {
                    //LogHelper.WriteExceptionToLog(String.Format("errorString: {0}\ne: {1}", e.Message, e.ToString()));
                    throw new BaseDatabaseException("GetDataTableAsync:" + e.Message, e);
                }
            }
            throw new NotImplementedException();
        }
